...
def sourceCodeGitUrl = "10.0.250.70:8088/android-boss/starda_newboss.git"

// 环境要求：外网下载gradle 3.5及相关jar包等 + android
node("base3-slave") {
    try {
        stage('generate tag') {
            // 生成tag时需要的时间标签
    ...
    if(buildStatus.contentEquals("unset")) {
        try {
            stage('upload building product to ftp') {
                sh"""
                    mv ${sourceCodeDir}/app/build/outputs/apk/app-release.apk ${sourceCodeDir}/app/build/outputs/apk/starda_newboss-${tag}.apk
