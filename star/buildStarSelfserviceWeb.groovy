...
// webself版本号
def webVersion = env.WEBVERSION.replace(" ", "")
// 取值：selfservice
def webExtParam = env.WEBEXTPARAM.replace(" ", "")
// 构建内容：主应用：APP 数据库脚本：DB 全部构建：BOTH
def buildContent = env.BUILDCONTENT.replace(" ", "")
// 国内：DOMESTIC 海外：ABROAD 目前只支持海外
...
node('base-slave') {
    ...
    if(buildStatus.contentEquals("unset")) {
        try {
            stage('prepare environment') {
                ...
                // 清空历史构建产物
                sh"""
                    rm -rf ./$webVersion/publish/*
                    rm -rf ./WEBSELFDBSCRIPTS/publish/*
                """
            }
        } catch(err) {
            buildStatus = "FALSE"
            ...
            stage('update code') {
                ...
                if(buildContent.contentEquals("BOTH") || buildContent.contentEquals("APP")) {
                    sh """
                        ...
                        if [ ! -d "./$webVersion/CVS" ]; then
                            cvs co -d $webVersion -r $branch boss/starsms-3.x/develop/4.implement/stariboss
                        else
                            cvs update -d -r $branch ./$webVersion
                        fi
                        if [ ! -d "./tool/CVS" ]; then
                ...
                if(buildContent.contentEquals("BOTH") || buildContent.contentEquals("DB")) {
                    sh """
                        ...
                        if [ ! -d "./DIRDBSCRIPTS/CVS" ]; then
                            cvs co -r HEAD -d WEBSELFDBSCRIPTS boss/starsms-3.x/develop/4.implement/dbscripts/WssDbScripts_Haiwai
                        else
                            cvs update -d -r HEAD ./WEBSELFDBSCRIPTS
                        fi
                    """
                }
            }
    ...
    if(buildStatus.contentEquals("unset")) {
        try {
            stage('build') {
                sh"""
                    rm -f ./$webVersion/stariboss_lib/*.jar
                    cp ./$webVersion/wss_jar/*.jar ./$webVersion/stariboss_lib/
                """
                if(area.contentEquals("ABROAD")) {
                    if(buildContent.contentEquals("BOTH") || buildContent.contentEquals("APP")) {
                        sh """
                            ...
                            /usr/apache-ant-1.9.6/bin/ant -file ${workspace}/${webVersion}/build.xml -Dbuilder=${builder} -Dbuild.dstamp=${nowDateStr} -Dbuild.number=${buildNum} -Dproject.ver=${webVersion} -Dmodule.ver=${webVersion} -Dbuild.extparam=${webExtParam} build.all
                        """
                    }
                    if(buildContent.contentEquals("BOTH") || buildContent.contentEquals("DB")) {
                        sh """
                            ...
                            /usr/apache-ant-1.9.6/bin/ant -file ${workspace}/WEBSELFDBSCRIPTS/build.xml -Dbuild.dstamp=${nowDateStr} -Dbuild.number=${buildNum} -Dbuilder=${builder} -Dproject.name=stariboss -Dproject.name.lowercase=stariboss -Dmodule.name=wssdbscripts_haiwai -Dmodule.name.lowercase=wssdbscripts_haiwai -Dproject.ver=${version} -Dmodule.ver=${webVersion} build.all
                        """
                    }
                } else {
                }
            ...
            stage('tag code') {
                ...
                if(buildContent.contentEquals("BOTH") || buildContent.contentEquals("APP")) {
                    sh """
                        export CVSROOT=:pserver:scm:scisnotcs@192.168.32.11:/cvsroot
                        cvs -d :pserver:scm:scisnotcs@192.168.32.11:/cvsroot login
                        cvs rtag -r ${branch} ${tag} boss/starsms-3.x/develop/4.implement/stariboss
                    """
                }
                if(buildContent.contentEquals("BOTH") || buildContent.contentEquals("DB")) {
                    sh """
                        export CVSROOT=:pserver:scm:scisnotcs@192.168.32.11:/cvsroot
                        cvs -d :pserver:scm:scisnotcs@192.168.32.11:/cvsroot login
                        cvs rtag -r HEAD ${tag} boss/starsms-3.x/develop/4.implement/dbscripts/WssDbScripts_Haiwai
                    """
                }
            }
        } catch(err) {
    ...
    if(buildStatus.contentEquals("unset")) {
        try {
            stage('upload building product to ftp') {
                if(buildContent.contentEquals("BOTH") || buildContent.contentEquals("APP")) {
                    sh """
                        echo "BUILDER  : ${builder}" > ./${webVersion}/publish/buildinfo.txt
                        echo "BRANCH   : ${branch}" >> ./${webVersion}/publish/buildinfo.txt
                    """
                    if(ifTag.contentEquals("Y")) {
                        sh """
                            echo "SRCTAG   : ${tag}" >> ./${webVersion}/publish/buildinfo.txt
                        """
                    }
                    op.uploadBuildingProductToFtp("${workspace}/${appSource}", "WEBSELFAPP", version, tag, pythonDir, appOut, "WEBSELFAPP", null)
                    op.uploadBuildingProductToFtp("${workspace}/${appSimSource}", "WEBSELFSIM", version, tag, pythonDir, appSimOut, "WEBSELFSIM", null)
                }
                if(buildContent.contentEquals("BOTH") || buildContent.contentEquals("DB")) {
                    sh """
                        echo "BUILDER  : ${builder}" > ./WEBSELFDBSCRIPTS/publish/buildinfo.txt
                    """
                    if(ifTag.contentEquals("Y")) {
                        sh """
                            echo "SRCTAG   : ${tag}" >> ./WEBSELFDBSCRIPTS/publish/buildinfo.txt
                        """
                    }
                    op.uploadBuildingProductToFtp("${workspace}/${dbSource}", "WEBSELFDB", version, tag, pythonDir, dbOut, "WEBSELFDB", null)
                }
            }
        } catch(err) {
            buildStatus = "FALSE"
