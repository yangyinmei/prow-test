...
def sourceCodeGitUrl = "10.0.250.70:8088/starott-stat/SADB.git"

// 环境要求：dos2unix + /usr/gradle-2.14.1 + zip
node("base-slave") {
    try {
        stage('generate tag') {
            // 生成tag时需要的时间标签
                ...
                stage('build') {
                    dir(codeDir) {
                        sh"""
                            ...
                            /usr/gradle-2.14.1/bin/gradle ${moduleName}
        ...
        if(buildStatus.contentEquals("unset")) {
            try {
                stage('upload building product to ftp') {
                    sh"""
                        echo "BUILDER  : ${builder}" > ${codeDir}/buildinfo.txt
                    """
                    if(ifTag.contentEquals("Y")) {
                        sh"""
                            echo "SRCTAG : ${tag}" >> ${codeDir}/buildinfo.txt
                        """
                    }
