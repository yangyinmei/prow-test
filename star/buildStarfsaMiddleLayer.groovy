...
def sourceCodeGitUrl = "10.0.250.70:8088/starfsa/starfsa.git"

// 环境要求：/usr/gradle-2.10
node("base-slave") {
    try {
        stage('generate tag') {
            // 生成tag时需要的时间标签
            ...
            stage('build') {
                dir(sourceCodeDir) {
                    sh"""
                        ...
                        /usr/gradle-2.10/bin/gradle clean assemble
                    """
                }
            }
        } catch(err) {
    ...
    if(buildStatus.contentEquals("unset")) {
        try {
            stage('upload building product to ftp') {
                sh"""
                    mv ${sourceCodeDir}/starFSA_appIntf/build/libs/starFSA_appIntf.war ${sourceCodeDir}/starFSA_appIntf/build/libs/starFSAAppIntf.war
                    echo "BUILDER  : ${builder}" > ${sourceCodeDir}/starFSA_appIntf/build/libs/buildinfo.txt
                """
                if(ifTag.contentEquals("Y")) {
                    sh"""
                        echo "SRCTAG : ${tag}" >> ${sourceCodeDir}/starFSA_appIntf/build/libs/buildinfo.txt
                    """
                }
                ...
                dir(sourceCodeDir) {
                    sh"""
                        if [ ! -d  sonar-project.properties ]; then
                            ...
                            echo "sonar.exclusions=**/common.properties,**/module.properties,**/*.properties,**/*.xml" >> ./sonar-project.properties
                            echo "sonar.java.binaries=./starFSA_appIntf" >> ./sonar-project.properties
                        fi
                        source /etc/profile
                        sonar-scanner
                    """
