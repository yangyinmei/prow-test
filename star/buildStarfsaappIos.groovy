...
def sourceCodeGitUrl = "10.0.250.70:8088/starfsa/starfsa-app-ios.git"
        ...
        try {
            stage('update code') {
                ...
                build job: 'UpdateSourceCode', parameters: [string(name: 'JOBNAME', value: jobName),
                                                            ...
                                                            string(name: 'GITURL', value: "${props_ott.starfsaappiosSourceCodeGitUrl}")]
            }
        } catch(err) {
            buildStatus = "FALSE"
            echo "${err}"
                ...
                dir(sourceCodeDir) {
                    sh """
                        ...
                        then
                            ...
                            echo '${tag} not exist.'
                            git config --local remote.origin.url http://${props_comm.gitUser}:${props_comm.gitPassword}@${props_ott.starfsaappiosSourceCodeGitUrl}
    ...
    if(buildStatus.contentEquals("unset")) {
        try {
            stage('upload building product to ftp') {
                dir(sourceCodeDir) {
                    sh"""
                        echo "BUILDER  : $BUILDER" > ./StarfsaAppHTML/dist/buildinfo.txt
                    """
                    if(ifTag.contentEquals("Y")) {
                        sh"""
                            echo "TAG      : $tag" >> ./StarfsaAppHTML/dist/buildinfo.txt
                        """
                    }
                }
