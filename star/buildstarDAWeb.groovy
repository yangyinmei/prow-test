...
def sourceCodeGitUrl = "10.0.250.70:8088/star_web/starDA-web.git"
            ...
            stage('build') {
                dir(sourceCodeDir) {
                    sh"""
                        ...
                        /usr/gradle-2.10/bin/gradle assemble
                    """
                }
            }
        } catch(err) {
    ...
    if(buildStatus.contentEquals("unset")) {
        try {
            stage('upload building product to ftp') {
                sh"""
                    rm -f ./build/libs/*.jar
                    echo "BUILDER  : ${builder}" > ${sourceCodeDir}/build/libs/buildinfo.txt
                """
                if(ifTag.contentEquals("Y")) {
                    sh"""
                        echo "SRCTAG : ${tag}" >> ${sourceCodeDir}/build/libs/buildinfo.txt
                    """
                }
