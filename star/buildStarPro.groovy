...
if(jobName.contentEquals("StarProLauncher")) {
    sourceCodeGitUrl = "10.0.250.70:8088/business-ott/projector_develop.git"
} else if(jobName.contentEquals("StarProSetting")) {
    sourceCodeGitUrl = "10.0.250.70:8088/business-ott/projector-setting.git"
} else if(jobName.contentEquals("StarProXiaoYi")) {
    sourceCodeGitUrl = "10.0.250.70:8088/business-ott/projector-xiaoyi.git"
}
    ...
    if(buildStatus.contentEquals("unset")) {
        try {
            stage('upload building product to ftp') {
                dir(sourceCodeDir) {
                    sh"""
                        mv ./app/build/outputs/apk/app-release.apk ./app/build/outputs/apk/${jobName}-${tag}.apk
                        echo "BUILDER  : $BUILDER" > ./app/build/outputs/apk/buildinfo.txt
                    """
                    if(ifTag.contentEquals("Y")) {
                        sh"""
                            echo "TAG      : $tag" >> ./app/build/outputs/apk/buildinfo.txt
                        """
                    }
                }
                ...
                dir(sourceCodeDir) {
                    sh"""
                        if [ ! -d  sonar-project.properties ]; then
                            ...
                            echo "sonar.java.binaries=./app" >> ./sonar-project.properties
                        fi
                        source /etc/profile
                        sonar-scanner
                    """
