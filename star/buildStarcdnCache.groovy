...
// 环境要求：dos2unix + make + ln -s /usr/bin/make /usr/bin/gmake + autoconf
node("192.168.32.170") {
    try {
        stage('generate tag') {
            // 生成tag时需要的时间标签
        ...
        try {
            stage('update code') {
                ...
                build job: 'UpdateSourceCode', parameters: [string(name: 'JOBNAME', value: jobName),
                                                            ...
                                                            string(name: 'GITURL', value: "${props_ott.starcdncacheSourceCodeGitUrl}")]
            }
        } catch(err) {
            buildStatus = "FALSE"
            echo "${err}"
        ...
        try {
            stage('build') {
                dir(sourceCodeDir) {
                    // 清空构建产物，再构建
                    sh """
                        rm -rf ${sourceCodeDir}/streamer/publish
                ...
                dir(sourceCodeDir) {
                    sh """
                        ...
                        then
                            ...
                            echo '${tag} not exist.'
                            git config --local remote.origin.url http://${props_comm.gitUser}:${props_comm.gitPassword}@${props_ott.starcdncacheSourceCodeGitUrl}
