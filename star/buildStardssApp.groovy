...
def sourceCodeGitUrl = "10.0.250.70:8088/stardss/stardss-app.git"

// 环境要求：/usr/gradle-2.10 + android 23.0.3 23.0.2
node("192.168.32.170") {
    try {
        stage('generate tag') {
            // 生成tag时需要的时间标签
        ...
        try {
            stage('build') {
                dir(sourceCodeDir) {
                    sh"""
                        #为gradle传递签名
                        echo systemProp.REPO_keyAlias=stardss > gradle.properties
                        ...
                        source /etc/profile
                        /usr/gradle-2.10/bin/gradle clean assemblerelease
                    """
                }
            }
        } catch(err) {
    ...
    if(buildStatus.contentEquals("unset")) {
        try {
            stage('upload building product to ftp') {
                sh"""
                    rm -f ${sourceCodeDir}/app/build/outputs/apk/app-release-unaligned.apk
                    mv ${sourceCodeDir}/app/build/outputs/apk/app-release.apk ${sourceCodeDir}/app/build/outputs/apk/app-release-${tag}.apk
