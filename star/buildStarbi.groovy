...
def sourceCodeGitUrl = "10.0.250.70:8088/starbi/starbi.git"

// 环境要求：/usr/apache-ant-1.9.6
node("base-slave") {
    try {
        stage('generate tag') {
            // 生成tag时需要的时间标签
    ...
    if(buildStatus.contentEquals("unset")) {
        try {
            stage('build') {
                dir(sourceCodeDir) {
                    sh"""
                        /usr/apache-ant-1.9.6/bin/ant -file ./build.xml -Dbuilder=${builder} -Dbuild.dstamp=${nowDateStr} -Dbuild.number=${buildNum} -Dproject.ver=${version} -Dmodule.ver=${version} build.all
                    """
                }
            }
        } catch(err) {
    ...
    if(buildStatus.contentEquals("unset")) {
        try {
            stage('upload building product to ftp') {
                sh"""
                    echo "BUILDER  : ${builder}" > ${sourceCodeDir}/publish/buildinfo.txt
                """
                if(ifTag.contentEquals("Y")) {
                    sh"""
                        echo "SRCTAG : ${tag}" >> ${sourceCodeDir}/publish/buildinfo.txt
                    """
                }
