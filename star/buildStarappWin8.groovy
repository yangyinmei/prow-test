...
def sourceCodeGitUrl = "10.0.250.70:8088/starapp/starapp-win8.git"

// 环境要求：/usr/gradle-2.14.1 + Android 24.0.3
node("192.168.32.170") {
    try {
        stage('generate tag') {
            // 生成tag时需要的时间标签
    ...
    if(buildStatus.contentEquals("unset")) {
        try {
            stage('build') {
                dir(sourceCodeDir) {
                    if(branch.contentEquals("Hotel_UGA")) {
                        sh"""
                            . /etc/profile
                            export GRADLE_USER_HOME=/home/gradleCaches
                            /usr/gradle-4.1/bin/gradle clean
                            /usr/gradle-4.1/bin/gradle build
                        """
                    } else {
                        sh"""
                            . /etc/profile
                            export GRADLE_USER_HOME=/home/gradleCaches
                            /usr/gradle-2.14.1/bin/gradle clean
                            /usr/gradle-2.14.1/bin/gradle build
                        """
                    }
                }
            }
    ...
    if(buildStatus.contentEquals("unset")) {
        try {
            stage('upload building product to ftp') {
                sh"""
                    echo "BUILDER  : ${builder}" > ${sourceCodeDir}/starbox_win8ui/build/outputs/apk/buildinfo.txt
                """
                if(ifTag.contentEquals("Y")) {
                    sh"""
                        echo "SRCTAG : ${tag}" >> ${sourceCodeDir}/starbox_win8ui/build/outputs/apk/buildinfo.txt
                    """
                }
                ...
                dir(sourceCodeDir) {
                    sh"""
                        if [ ! -d  sonar-project.properties ]; then
                            ...
                            echo "sonar.exclusions=**/starbox_win8ui/build/outputs/*.xml" >> ./sonar-project.properties
                            echo "sonar.java.binaries=./starbox,./starbox_win8ui,./starbox_uiframework" >> ./sonar-project.properties
                        fi
                        source /etc/profile
                        sonar-scanner
                    """
