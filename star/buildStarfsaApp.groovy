...
def sourceCodeGitUrl = "10.0.250.70:8088/starfsa/starfsa-app.git"

// 环境要求：外网下载gradle 3.3及相关jar包等
node("192.168.32.170") {
    try {
        stage('generate tag') {
            // 生成tag时需要的时间标签
        ...
        try {
            stage('build') {
                dir(sourceCodeDir) {
                    sh"""
                        #为gradle传递签名
                        echo systemProp.REPO_keyAlias=Starfsa > gradle.properties
    ...
    if(buildStatus.contentEquals("unset")) {
        try {
            stage('upload building product to ftp') {
                sh"""
                    rm -rf ${sourceCodeDir}/app/build/outputs/apk/*-unaligned.apk
