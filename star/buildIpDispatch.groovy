...
def sourceCodeGitUrl = "10.0.250.70:8088/star_web/ip-dispatch.git"
            ...
            stage('build') {
                dir(sourceCodeDir) {
                    sh"""
                        ...
                        /usr/gradle-2.10/bin/gradle clean -b ./ip-dispatch/build.gradle
                        /usr/gradle-2.10/bin/gradle assemble -b ./ip-dispatch/build.gradle
                    """
                }
            }
        } catch(err) {
    ...
    if(buildStatus.contentEquals("unset")) {
        try {
            stage('upload building product to ftp') {
                sh"""
                    rm -f ${sourceCodeDir}/ip-dispatch/build/libs/*.jar
                    echo "BUILDER  : ${builder}" > ${sourceCodeDir}/ip-dispatch/build/libs/buildinfo.txt
                """
                if(ifTag.contentEquals("Y")) {
                    sh"""
                        echo "SRCTAG : ${tag}" >> ${sourceCodeDir}/ip-dispatch/build/libs/buildinfo.txt
                    """
                }
