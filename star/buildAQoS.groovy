...
def sourceCodeGitUrl = "10.0.250.70:8088/AQoS/AQoS.git"
...
node('base-slave') {
        ...
        try {
            stage('update code') {
                ...
                build job: 'UpdateSourceCode', parameters: [string(name: 'JOBNAME', value: jobName),
                                                            ...
                                                            string(name: 'GITURL', value: "${props_ott.aqosSourceCodeGitUrl}")]
            }
        } catch(err) {
            buildStatus = "FALSE"
            echo "${err}"
            ...
            stage('build') {
                dir(sourceCodeDir){
                    sh"""
                        ...
                        /usr/gradle-2.10/bin/gradle clean zip_aqos
                    """
                }
            }
        } catch(err) {
                ...
                dir(sourceCodeDir) {
                    sh """
                        ...
                        then
                            ...
                            echo '${tag} not exist.'
                            git config --local remote.origin.url http://${props_comm.gitUser}:${props_comm.gitPassword}@${props_ott.aqosSourceCodeGitUrl}
    ...
    if(buildStatus.contentEquals("unset")) {
        try {
            stage('upload building product to ftp') {
                dir(sourceCodeDir) {
                    sh"""
                        echo "BUILDER  : $BUILDER" > ./publish/dist/buildinfo.txt
                    """
                    if(ifTag.contentEquals("Y")) {
                        sh"""
                            echo "TAG      : $tag" >> ./publish/dist/buildinfo.txt
                        """
                    }
                }
    ...
    stage('send email') {
        ...
        def bodyContent = """
            ...
            <li>程序构建包&nbsp;：<a href="ftp://10.0.250.250/${jobName}/${version}/${tag}/${jobName}/">ftp://10.0.250.250/${jobName}/${version}/${tag}/${jobName}/</a></li>
