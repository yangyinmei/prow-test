...
def sourceCodeGitUrl = "10.0.250.70:8088/stardss/stardss.git"

// 环境要求：/usr/java/jdk1.7.0_51
node("base-slave") {
    try {
        stage('generate tag') {
            // 生成tag时需要的时间标签
            ...
            stage('build') {
                dir(sourceCodeDir) {
                    sh"""
                        ...
                        /usr/apache-maven3.3.9/bin/mvn -f pom.xml clean package -Dbuildtimes=${buildNum} -Dbuilddate=${nowDateStr}
                    """
                }
            }
        } catch(err) {
    ...
    if(buildStatus.contentEquals("unset")) {
        try {
            stage('upload building product to ftp') {
                sh"""
                    echo "BUILDER  : ${builder}" >> ${sourceCodeDir}/target/buildinfo.txt
                """
                if(ifTag.contentEquals("Y")) {
                    sh"""
                        echo "SRCTAG : ${tag}" >> ${sourceCodeDir}/target/buildinfo.txt
                    """
                }
